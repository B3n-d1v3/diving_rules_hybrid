// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'diving_rules_localizations.dart';

// ignore_for_file: type=lint

/// The translations for French (`fr`).
class AppLocalizationsFr extends AppLocalizations {
  AppLocalizationsFr([String locale = 'fr']) : super(locale);

  @override
  String get navigationMenuRules => 'Règles';

  @override
  String get navigationMenuPenalties => 'Pénalités';

  @override
  String get navigationMenuQuizz => 'Questionnaire';

  @override
  String get navigationMenuSearch => 'Recherche';

  @override
  String get penaltiesListTitle => 'Cas de pénalités';

  @override
  String get buttonJudge => 'Juge';

  @override
  String get penaltyRule => 'Règle';

  @override
  String get penaltyDescription => 'Description';

  @override
  String get penaltyPenalty => 'Pénalité';

  @override
  String get penaltyOwnership => 'Par qui';

  @override
  String get button0pts => '0 pts';

  @override
  String get buttonMax2pts => 'max 2 pts';

  @override
  String get buttonMax4Halfpts => 'max 4 ½ pts';

  @override
  String get buttonMinus2pts => '-2 pts';

  @override
  String get buttonMinusHalfTo2pts => '- ½ à 2 pts';

  @override
  String get buttonJudgeOpinion => 'selon opinion';

  @override
  String get buttonReferee => 'Arbitre';

  @override
  String get quizzIntroTitle => 'Testez vos connaissances';

  @override
  String get quizzIntroDescription =>
      'Afin de vous entraîner sur les règles World Aquatics du plongeon, avec ce test, vous serez en mesure de réviser les règles du résumé des pénalités avec des questions sur les différentes règles.\nCe test n\'est pas suffisant pour passer l\'examen du juge, puisqu\'il est concentré sur le résumé des pénalités, mais il aide a apprendre les sanctions.\n\nSélectionnez le nombre de questions que vous souhaitez dans votre quiz (entre 5 et 40).';

  @override
  String get quizzIntroStartButton => 'Démarrer';

  @override
  String get quizzNext => 'Suivant';

  @override
  String get cFBundleDisplayName => 'diving rules';

  @override
  String get cFBundleName => 'diving rules';

  @override
  String get quizzTitle => 'Questionnaire';

  @override
  String get quizzResultTitle => 'Résultat du Quizz';

  @override
  String get quizzResultDescription =>
      'Voici votre résultat au questionnaire : ';

  @override
  String get quizzResultStartButton => 'Nouveau';

  @override
  String get passed => 'Réussi';

  @override
  String get failed => 'Echoué';

  @override
  String get searchTitle => 'Rechercher les Règles';

  @override
  String get ruleBookTitle => 'Règlement';

  @override
  String get quizzQuestion => 'Question';

  @override
  String get quizzResultReviewButton => 'Erreurs';

  @override
  String get quizzResultLegendUnused => 'Non utilisé';

  @override
  String get quizzResultLegendUserAnswer => 'Ma réponse';

  @override
  String get quizzResultLegendRealAnswer => 'Bonne Réponse';

  @override
  String get penalty0 =>
      'Si un plongeur ne plonge pas dans la minute qui suit l’avertissement du juge-arbitre.';

  @override
  String get penalty1 =>
      'Si, dans un plongeon au tremplin, un plongeur effectue un double rebond pendant l’élan, ou à l’extrémité du tremplin.';

  @override
  String get penalty2 =>
      'Si, dans un plongeon avec élan, le dernier pas (ou saut) avant l’envol final à deux pieds, se fait également à deux pieds.';

  @override
  String get penalty3 =>
      'Si l’envol, depuis le tremplin ou la plateforme, ne se fait pas simultanément des deux pieds.';

  @override
  String get penalty4 =>
      'Si la vrille, au moment de l’entrée à l’eau, est supérieure ou inférieure de 90º ou plus, par rapport à ce qui a été annoncé.';

  @override
  String get penalty5 =>
      'Si un plongeur ou une plongeuse a exécuté un plongeon d’un numéro différent de celui du plongeon annoncé.';

  @override
  String get penalty6 =>
      'Si, dans un plongeon par la tête, les pieds entrent dans l’eau avant la tête ou les mains.';

  @override
  String get penalty7 =>
      'Si, dans un plongeon par les pieds, la tête ou les mains entrent dans l’eau avant les pieds.';

  @override
  String get penalty8 =>
      'Si un Athlète reçoit une aide pendant l’exécution d’un plongeon, après le signal du juge-arbitre.';

  @override
  String get penalty9 =>
      'Lorsqu’une seconde tentative (un nouveau départ) est infructueuse.';

  @override
  String get penalty10 =>
      'Si un plongeur refuse ou ne parvient pas à exécuter un plongeon.';

  @override
  String get penalty11 =>
      'En plongeon synchronisé, si un plongeur, ou les deux plongeurs, exécutent un plongeon d’un numéro différent de ce qui a été annoncé.';

  @override
  String get penalty12 =>
      'En plongeon synchronisé, si tous les juges d’exécution pour un plongeur attribuent la note zéro (0).';

  @override
  String get penalty13 =>
      'En plongeon synchronisé, si tous les juges de synchronisation attribuent la note 0.';

  @override
  String get penalty14 =>
      'En plongeon synchronisé, si un plongeur, ou les deux plongeurs, exécutent un plongeon d’une position différente de ce qui a été annoncé.';

  @override
  String get penalty15 =>
      'En plongeon synchronisé, si un plongeur entre dans l’eau avant que l’autre n’ait quitté le tremplin ou la plateforme.';

  @override
  String get penalty16 =>
      'Si, lors d’un plongeon avec élan, un plongeur fait un ou plusieurs pas pendant son élan, puis marque un arrêt.';

  @override
  String get penalty17 =>
      'Lorsqu’il y a un nouveau départ, pour un plongeon avec élan, sans élan, ou en équilibre.';

  @override
  String get penalty18 =>
      'En plongeon synchronisé, s’il y a un nouveau départ, qu’il soit imputable ou dû à l’un ou les deux plongeurs.';

  @override
  String get penalty19 =>
      'Si un plongeur exécute un plongeon dans une position clairement différente de celle annoncée.';

  @override
  String get penalty20 =>
      'Si un plongeur est dangereusement près du tremplin ou de la plateforme, ou s’il touche l’extrémité du tremplin ou de la plateforme avec la tête.';

  @override
  String get penalty21 =>
      'Si, dans un plongeon par les pieds, un ou les deux bras sont tenus au-dessus de la tête.';

  @override
  String get penalty22 =>
      'Si, dans un plongeon pris au vol, la position droite n’est pas montrée pendant au moins un quart de saut périlleux (90º) pour les plongeons comportant jusqu’à 1 saut périlleux.';

  @override
  String get penalty23 =>
      'Si, dans un plongeon pris au vol, la position droite n’est pas montrée pendant au moins un demi-saut périlleux (180º) pour les plongeons comportant plus de 1 saut périlleux.';

  @override
  String get penalty24 =>
      'Si un plongeon n’est pas réalisé dans une position telle que décrite par ces règlements.';

  @override
  String get penalty25 =>
      'Si la position de départ n’est pas le corps droit, la tête droite et orientée vers l’avant, les bras tendus le long du corps, ou perpendiculairement au corps, ou au-dessus de la tête.';

  @override
  String get penalty26 =>
      'Si, pour un plongeon sans élan, les pieds du plongeur sont décollés du tremplin ou de la plateforme avant l’envol.';

  @override
  String get penalty27 =>
      'Si, dans la position de départ d’un plongeon en équilibre, le plongeur est incapable de maintenir un équilibre immobile et stable, à la verticale.';

  @override
  String get penalty28 =>
      'Dans un plongeon avec élan, si l’élan n’est pas fluide, esthétiquement plaisant, et en direction de l’avant, jusqu’à l’extrémité du tremplin ou de la plateforme.';

  @override
  String get penalty29 =>
      'Si, dans un plongeon avec ou sans élan, l’envol n’est pas équilibré et haut, et depuis l\'extrémité du tremplin ou de la plateforme.';

  @override
  String get penalty30 =>
      'Si, dans un plongeon avec vrille, la vrille est manifestement déclenchée à partir du tremplin ou de la plateforme.';

  @override
  String get penalty31 =>
      'Si la position droite n’est pas réalisée telle que décrite (corps droit, non plié ni aux genoux ni aux hanches, pieds serrés, pointes de pieds en extension), ou si elle n’est pas esthétiquement plaisante.';

  @override
  String get penalty32 =>
      'Si la position carpée n’est pas réalisée telle que décrite (corps plié aux hanches, jambes en extension, pieds serrés et pointes de pieds tendues), ou si elle n’est pas esthétiquement plaisante.';

  @override
  String get penalty33 =>
      'Si, dans un plongeon carpé avec vrille, la position carpée n’est pas clairement montrée.';

  @override
  String get penalty34 =>
      'Si la position groupée n’est pas réalisée telle que décrite (corps compact, plié aux genoux et aux hanches, avec les genoux et les pieds rapprochés ne dépassant pas la largeur des épaules, mains sur les tibias, pointes de pieds tendues), ou si elle n’est pas esthétiquement plaisante.';

  @override
  String get penalty35 =>
      'Si, dans un plongeon groupé avec vrille, la position groupée n’est pas clairement montrée.';

  @override
  String get penalty36 =>
      'Si la position libre n’est pas réalisée telle que décrite (position optionnelle entre droit, carpée ou groupé,  jambes serrées, et pointes de pieds tendues).';

  @override
  String get penalty37 =>
      'Si les bras ne sont pas dans une position correcte lors d’une entrée à l’eau par la tête ou par les pieds.';

  @override
  String get penalty38 =>
      'En plongeon synchronisé, pour un manque de similitude de la position de départ, de l\'élan, de l\'envol et de la hauteur.';

  @override
  String get penalty39 =>
      'En plongeon synchronisé, pour un manque de coordination du rythme des mouvements durant la trajectoire.';

  @override
  String get penalty40 =>
      'En plongeon synchronisé, pour un manque de similitude des angles verticaux des entrées à l\'eau.';

  @override
  String get penalty41 =>
      'En plongeon synchronisé, pour un manque de similitude des distances par rapport au tremplin ou à la plate-forme, lors de l\'entrée dans l\'eau.';

  @override
  String get penalty42 =>
      'En plongeon synchronisé, pour un manque de simultaniété des entrées à l\'eau.';

  @override
  String get penalty43 =>
      'Si un plongeon est exécuté partiellement dans une position différente de celle annoncée.';

  @override
  String get penalty44 =>
      'Si, pendant la trajectoire, un plongeur se déporte de sa ligne directe d’envol.';

  @override
  String get penalty45 =>
      'Si, pendant la trajectoire, un plongeur touche l’extrémité du tremplin ou de la plateforme, avec les pieds ou avec les mains.';

  @override
  String get penalty46 =>
      'Si l’entrée à l’eau est plate ou passée, dans un angle inférieur ou supérieur à la verticale.';

  @override
  String get penalty47 =>
      'Si au moment de l’entrée à l’eau le corps est vrillé.';

  @override
  String get penalty48 =>
      'Si au moment de l’entrée à l’eau le corps n\'est pas droit.';

  @override
  String get penalty49 =>
      'Si au moment de l’entrée à l’eau les pieds sont désunis, ou si les pointes de pieds sont relâchées.';

  @override
  String get penalty50 =>
      'Si au moment de l’entrée à l’eau la surface de l’eau est significativement perturbée.';

  @override
  String get quizzIntroQuestionNumber => 'Nombre de questions : ';

  @override
  String get aboutTitle => 'A propos de';

  @override
  String get aboutVersion => 'Version : ';

  @override
  String get aboutDescription =>
      'Cette application a été créée pour vous permettre d\'apprendre et de s\'entraîner sur les règles de plongeon à partir du règlement de la World Aquatics.\nCette application n\'est pas une application officielle de la World Aquatics, mais utilise le règlement officiel de la World Aquatics ou ses traductions officielles comme outil de travail.\n\n Avec cette application, vous pouvez :\n✔︎ Retrouver le règlement dans votre langue.\n✔︎ Réviser le résumé des penalités.\n✔︎ Vous entrainer avec un quizz sur le résumé des pénalités.\n✔︎ Choisir de réviser l\'ensemble en Anglais.';

  @override
  String get aboutFeedbackLink => 'Suggestions';

  @override
  String get aboutRulesLocal => 'Règlement Francais: FFN - 18 fev 2026';

  @override
  String get aboutRulesReference =>
      'Règlement de l\'app : World Aquatics - 18 fev 2026';

  @override
  String get aboutLicense =>
      'La version originale a été créée en 2021 par Benoît SUZANNE sous la licence GPL 3.0 open source. Cette licence est la GNU General Public License, une licence libre de type copyleft.';

  @override
  String get aboutThanks =>
      'Remerciements à :\n• Daniel AZORIN pour son soutien dans la finalisation de l\'appli et pour la traduction en espagnol.\n• Piero ITALIANI pour la traduction italienne.\n• Macarena SALGADO PATON et Daniel AZORIN pour la traduction espagnol\n• Thomas HAUSOTTER pour la traduction allemande';

  @override
  String get aboutHelp =>
      'N\'hésitez pas à nous contacter :\n• Si vous êtes prêt à aider pour les nouvelles mises à jour de traductions.\n• Pour toutes questions, commentaires et idées de nouvelles fonctionnalités.\n';

  @override
  String get aboutDescriptionTitle => 'A propos de l\'application';

  @override
  String get aboutWADivingLink => 'Plongeon (🇬🇧)';

  @override
  String get aboutWARulesLink => 'Dernières règles (🇬🇧)';

  @override
  String get aboutLicenseTitle => 'License';

  @override
  String get aboutThanksTitle => 'Remerciements';

  @override
  String get aboutFeedbackTitle => 'Dite m\'en plus';

  @override
  String get aboutShareTitle => 'Partage';

  @override
  String get aboutShare => 'Partagez cette application';

  @override
  String get quizzQuestionHeader => 'Questions';

  @override
  String get quizzResultHeader => 'Résultat';

  @override
  String get quizzCorrectionListHeader => 'Corrections';

  @override
  String get quizzCorrectionDetailHeader => 'Correction';

  @override
  String get quizzQuestionWarningNext =>
      'Sélectionnez la pénalité et qui l\'applique anvant de passer à la question suivante.';

  @override
  String get rulebookCopy => 'Copie';

  @override
  String get rulebookBookmarks => 'Signet';

  @override
  String get rulebookSearch => 'Recherche';

  @override
  String get rulebookSearchNoResult => 'Aucun résultat trouvé';

  @override
  String get rulebookSearchHintText => 'Recherche...';

  @override
  String get rulebookSearchPrevious => 'Précédent';

  @override
  String get rulebookSearchNext => 'Suivant';

  @override
  String get rulebookSearchCancel => 'Annuler la recherche';

  @override
  String get languageSelection => 'Langue';

  @override
  String get previous => 'Précédente';

  @override
  String get next => 'Suivante';
}
